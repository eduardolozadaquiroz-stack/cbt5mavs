...
export default function SeleccionarAlumnoPage() {
  ...
  useEffect(() => {
    async function fetchMisAlumnos() {
      try {
        const res = await fetch("/api/padres/mis-alumnos", {
          credentials: "include", // envía las cookies de sesión JWT
        });
        ...
        if (!res.ok) {
          setError(json.error ?? "Error al cargar alumnos vinculados.");
          return;
        }

        setAlumnos(json.alumnos ?? []);
        if (json.alumnos?.length === 1) {
          // Si solo hay un alumno, preseleccionarlo
          setSelectedId(json.alumnos[0].alumno_id);
        }
      } catch {
          ...
          {loadingList ? (
            ...
            <div className="rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-700 px-4 py-4 text-center">
              ...
              <p className="font-body-sm text-body-sm text-amber-700 dark:text-amber-300">
                Tu cuenta no tiene ningún estudiante asignado. Contacta a la administración del plantel para registrar a tu hijo/a.
              </p>
              <Link
                href="/contacto"
