...
export default function GruposPage() {
  ...
  const [fCarrera, setFCarrera]   = useState<Carrera | "Todos">("Todos");
  const [fSem, setFSem]           = useState<number>(0);    // 0 = todos
  const [fTurno, setFTurno]       = useState<Turno | "Todos">("Todos");
  const [query, setQuery]         = useState("");

  const filtrados = grupos.filter((g) =>
    (fCarrera === "Todos" || g.carrera === fCarrera) &&
    (fSem === 0 || g.semestre === fSem) &&
    (fTurno === "Todos" || g.turno === fTurno) &&
    (query === "" || g.id.toLowerCase().includes(query.toLowerCase()) ||
      g.tutor.toLowerCase().includes(query.toLowerCase()) ||
      g.salon.toLowerCase().includes(query.toLowerCase()))
  );

  const totalAlumnos = grupos.reduce((a, g) => a + g.alumnos, 0);
  const filtTotal    = filtrados.reduce((a, g) => a + g.alumnos, 0);

  return (
    <>
      <DashboardTopbar userImageAlt="Administrador" userName="Mtra. Viderique" userRole="Administradora" activeTopLink="grupos" showSearch linkBase={BASE} />
        ...
        <main className="flex-1 md:ml-64 p-4 md:p-5 lg:p-6 max-w-[1280px] mx-auto w-full">
          ...
          <div className="mb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">Ciclo 2025-2026 — {grupos.length} grupos · {totalAlumnos.toLocaleString()} alumnos inscritos</p>
            </div>
            <button onClick={() => setModal(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-700 text-white text-sm font-semibold rounded-lg hover:bg-blue-800 transition-colors self-start sm:self-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
            {[
              ...
              { label: "Total alumnos",    val: totalAlumnos.toLocaleString(),                               color: "bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-200" },
              { label: "Turno Matutino",   val: `${grupos.filter((g) => g.turno === "Matutino").length} gps`, color: "bg-sky-50 dark:bg-sky-950/20 text-sky-700 dark:text-sky-300" },
              { label: "Turno Vespertino", val: `${grupos.filter((g) => g.turno === "Vespertino").length} gps`, color: "bg-indigo-50 dark:bg-indigo-950/20 text-indigo-700 dark:text-indigo-300" },
            ].map(({ label, val, color }) => (
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-3 mb-3 flex flex-col gap-2.5">
            ...
            <div className="relative">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar por ID de grupo, tutor o salón..." className="w-full sm:w-80 pl-9 pr-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-300" />
            </div>
            {/* Carrera */}
            <div className="flex gap-1.5 flex-wrap items-center">
              <span className="text-xs font-semibold text-slate-400 mr-1">Carrera:</span>
              {(["Todos","Informática","Gastronomía","CAD"] as (Carrera|"Todos")[]).map((c) => (
                <button key={c} onClick={() => setFCarrera(c)} className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border transition-colors ${fCarrera === c ? "bg-blue-700 text-white border-blue-700" : "border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>
                  {c === "Todos" ? "Todas" : c === "CAD" ? "Diseño CAD" : c}
                </button>
              ))}
            </div>
            {/* Semestre */}
            <div className="flex gap-1.5 flex-wrap items-center">
              ...
              <span className="ml-2 text-xs text-slate-400">(S1-S2 = 1er año · S3-S4 = 2do año · S5-S6 = 3er año)</span>
            </div>
            {/* Turno */}
            <div className="flex gap-1.5 flex-wrap items-center">
              <span className="text-xs font-semibold text-slate-400 mr-1">Turno:</span>
              {(["Todos","Matutino","Vespertino"] as (Turno|"Todos")[]).map((t) => (
                <button key={t} onClick={() => setFTurno(t)} className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border transition-colors ${fTurno === t ? "bg-sky-700 text-white border-sky-700" : "border-slate-200 dark:border-slate-600 text-slate-600 hover:bg-slate-50"}`}>
                  {t === "Todos" ? "Ambos turnos" : t}
                </button>
              ))}
            </div>
          </div>
              ...
              <table className="w-full border-collapse min-w-[820px]">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    <th className="px-4 py-2.5 text-left border-b border-slate-200 dark:border-slate-700">ID de grupo</th>
                    <th className="px-4 py-2.5 text-left border-b border-slate-200 dark:border-slate-700">Carrera</th>
                    <th className="px-4 py-2.5 text-center border-b border-slate-200 dark:border-slate-700">Sem.</th>
                    <th className="px-4 py-2.5 text-center border-b border-slate-200 dark:border-slate-700">Gpo.</th>
                    <th className="px-4 py-2.5 text-center border-b border-slate-200 dark:border-slate-700">Turno</th>
                    <th className="px-4 py-2.5 text-left border-b border-slate-200 dark:border-slate-700">Tutor</th>
                    <th className="px-4 py-2.5 text-left border-b border-slate-200 dark:border-slate-700">Salón</th>
                    <th className="px-4 py-2.5 text-right border-b border-slate-200 dark:border-slate-700">Alumnos</th>
                    <th className="px-4 py-2.5 text-right border-b border-slate-200 dark:border-slate-700">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  {filtrados.length === 0 ? (
                    <tr><td colSpan={9} className="px-4 py-10 text-center text-sm text-slate-400">No hay grupos que coincidan con los filtros.</td></tr>
                  ) : filtrados.map((g) => (
                    <tr key={g.id} className="border-b border-slate-100 dark:border-slate-800 last:border-b-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-4 py-2.5 font-mono text-xs font-semibold text-slate-600 dark:text-slate-300">{g.id}</td>
                      <td className="px-4 py-2.5"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${CAR_COLOR[g.carrera]}`}>{g.carrera === "CAD" ? "Diseño CAD" : g.carrera}</span></td>
                      <td className="px-4 py-2.5 text-center font-bold text-xs text-slate-600 dark:text-slate-400">S{g.semestre}</td>
                      <td className="px-4 py-2.5 text-center font-bold text-slate-700 dark:text-slate-300">{g.letra}</td>
                      <td className="px-4 py-2.5 text-center"><span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-semibold ${TURNO_COLOR[g.turno]}`}>{g.turno === "Matutino" ? "Mat." : "Ves."}</span></td>
                      <td className="px-4 py-2.5 text-sm text-slate-600 dark:text-slate-400 max-w-[200px] truncate" title={g.tutor}>{g.tutor}</td>
                      <td className="px-4 py-2.5 text-sm text-slate-500 dark:text-slate-400">{g.salon}</td>
                      <td className="px-4 py-2.5 text-right font-semibold text-slate-700 dark:text-slate-200">{g.alumnos}</td>
                      <td className="px-4 py-2.5 text-right">
                        <a href={`${BASE}/grupos/${g.id}`} className="text-xs text-blue-700 dark:text-blue-400 hover:underline font-medium">Ver →</a>
                      </td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-slate-50 dark:bg-slate-800 text-xs text-slate-500">
                    <td colSpan={7} className="px-4 py-2 border-t border-slate-200 dark:border-slate-700">Mostrando {filtrados.length} de {grupos.length} grupos</td>
                    <td className="px-4 py-2 text-right border-t border-slate-200 dark:border-slate-700 font-semibold text-slate-700 dark:text-slate-300">{filtTotal.toLocaleString()} alumnos</td>
      ...
      {modal && (
        <ModalNuevoGrupo
          ...
          onCreated={(g) => { setGrupos((p) => [...p, g]); setModal(false); }}
        />
      )}
    </>
  );
